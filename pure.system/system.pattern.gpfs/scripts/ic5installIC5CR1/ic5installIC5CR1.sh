...
<repository location='${INSTCR1DIR}'/>
</server>
...
<data key='user.job' value='UPDATE'/>
...
<data key='user.notification.port' value='25'/>
<data key='user.notification.user' value=''/>
<data key='user.notification.password' value=''/>
...
<data key='user.ccm.new.add' value='false'/>
...
<data key='user.metrics.new.add' value='false'/>
<data key='user.moderation.new.add' value='false'/>
<data key='user.ccm.selected' value='false'/>
...
<install modify='false'>
<offering id='com.ibm.lotus.connections' version='5.0.0.0_20141013_1847' profile='IBM Connections' features='homepage,news,search,activities,blogs,communities,dogear,files,forum,metrics,mobile,moderation,profiles,wikis' installFixes='none'/>
</install>
...
EOF
) > ${basepath}/ic5_installCR1.rsp
}

setPermSharedDir()
{
markup starting "Setting permissions of IC Shared directory"
## we need to know, if GPFS or NFS is used, due to the linked GPFS fileset, where we cannot set permissons on the Link :-(
if [ "${Link_Directory}" == "" ]; then
  echo "GPFS is not used in this deployment"
  echo "Setting permissions for $RTUSER directly on the IC Share --> ${ICSHAREDDIR}"
  echo "chown -R ${RTUSR} ${MOUNT_POINT}/icdata/${PROJECTNAME}"
  chown -R ${RTUSR} ${MOUNT_POINT}/icdata/${PROJECTNAME}
  chmod -R 777 ${MOUNT_POINT}/icdata/${PROJECTNAME}
else
  echo "GPFS is used in this deployment"
  echo "We have to check, if the GPFS Share is used as Shared Directory...."
  CHECK1=`echo ${ICSHAREDDIR} |grep ${Link_Directory}`
  if [ "${CHECK1}" == "" ]; then
    echo "The GPFS Share is not used for IBM Connections Shared Directory!"
  else
	echo "The GPFS Share used for IBM Connections Shared Directory!"
	echo "Setting permissions on the device, not on the link"
	echo "chown -R ${RTUSR} /gpfs/gpfsdev/${FileSet_Name}"
	chown -R ${RTUSR} /gpfs/gpfsdev/${FileSet_Name}
	chmod -R 777 /gpfs/gpfsdev/${FileSet_Name}
  fi
fi
...
inst()
{
markup starting "Silent Installation of IBM Connections CR1"
echo "su - ${RTUSR} -c \"cd ${IMPath} && ./imcl -acceptLicense -input ${basepath}/ic5_installCR1.rsp -log /tmp/lc_silent_install.log\""
chown -R ${RTUSR} ${INSTLOC}
su - ${RTUSR} -c "cd ${IMPath} && ./imcl -acceptLicense -input ${basepath}/ic5_installCR1.rsp -log /tmp/lc_silent_install.log"
markup finished "Silent Installation of IBM Connections CR1"
}
...
PYSCRIPT=wasLDAPconfig.py
if [ "${LPORT}" == "636" ]; then
OPTS="ldapHost='"${LDAPSRV}"' ldapPort='"${LPORT}"' ldapUser='"${BINDUSR}"' ldapPassword='"${BINDPW}"' ldapType='"${LTYPE}"' ldapSSL='true'"
else
OPTS="ldapHost='"${LDAPSRV}"' ldapPort='"${LPORT}"' ldapUser='"${BINDUSR}"' ldapPassword='"${BINDPW}"' ldapType='"${LTYPE}"'"
fi
