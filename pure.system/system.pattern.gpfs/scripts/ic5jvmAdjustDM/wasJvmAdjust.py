...
def setJvmHeapSizes(node):
		
	memory = 2048
	log.info("Setting heapsize for DM: %d" %(memory))
		
	nodeID = lb.getNodeId(node)
	jvm = AdminConfig.list('JavaVirtualMachine', nodeID)
...
def _setJvmHeapSize(jvm, heapSize):
	...
	ret = AdminConfig.modify(jvm, [['initialHeapSize', '512'], ['maximumHeapSize', heapSize]])
