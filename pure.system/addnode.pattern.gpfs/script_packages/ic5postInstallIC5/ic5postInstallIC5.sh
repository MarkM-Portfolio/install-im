...
dos2unix ${WASTMP}
chown ${RTUSR} ${WASTMP}
echo "su - ${RTUSR} -c "cd ${WASPATH}/bin/ && ./wsadmin.sh ${WSOPT} -f ${WASTMP} -javaoption -Dpython.path=${basepath}""
su - ${RTUSR} -c "cd ${WASPATH}/bin/ && ./wsadmin.sh ${WSOPT} -f ${WASTMP} -javaoption -Dpython.path=${basepath}"
#rm ${WASTMP}
}
...
echo "su - ${RTUSR} -c \"cd ${basepath} && ${DMGRP}/bin/wsadmin.sh ${WSOPT} -f ${PYSCRIPT} -javaoption -Dpython.path=${basepath} $*\""
...
statusDMGR()
{
su -c ${RTUSR} -c "${WASPATH}/bin/${1}Manager.sh"
}
...
wasPlugin()
{
markup starting "Generate and Propagate Plugin"
PYSCRIPT=wasPlugin.py
OPTS="dmgrProfile=${DMGRP}"
execPYscript ${OPTS}
markup stopping "Generate and Propagate Plugin"
}
...
OPTS="hostName=${HTTPHOST} alias=http_ssl"
execPYscript ${OPTS}
...
cd ${DIRP} && /usr/bin/python ./$PYSCRIPT -f ${WASPATH}/config/cells/${CELLN}/LotusConnections-config/LotusConnections-config.xml -o `hostname -f` -n ${CONNECTIONSURL}
...
case $1 in
	...
	wasPlugin				# generate and propagte new Plugin
