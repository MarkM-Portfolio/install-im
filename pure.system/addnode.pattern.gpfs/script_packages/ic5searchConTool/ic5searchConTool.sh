...
if [ -d ${INSTLOC}/data/local/search/stellent/dcs/oiexport ]; then
 su -c ${RTUSR} -c "cp -Rfp ${ICSHAREDDIR}/search/stellent/dcs/oiexport/* ${INSTLOC}/data/local/search/stellent/dcs/oiexport/"
 echo "su -c ${RTUSR} -c "cp -Rfp ${ICSHAREDDIR}/search/stellent/dcs/oiexport/ ${INSTLOC}/data/local/search/stellent/dcs/oiexport/""
 else
 echo "su -c ${RTUSR} -c "mkdir -p ${INSTLOC}/data/local/search/stellent/dcs/oiexport""
 echo "su -c ${RTUSR} -c "cp -Rfp ${ICSHAREDDIR}/search/stellent/dcs/oiexport/ ${INSTLOC}/data/local/search/stellent/dcs/oiexport/""
 su -c ${RTUSR} -c "mkdir -p ${INSTLOC}/data/local/search/stellent/dcs/oiexport"
 su -c ${RTUSR} -c "cp -Rfp ${ICSHAREDDIR}/search/stellent/dcs/oiexport/* ${INSTLOC}/data/local/search/stellent/dcs/oiexport/"
fi
...
app_server_root=/opt/IBM/WebSphere/Profiles/DefaultCustom01
