...
execPYscript() {
 ...
 echo "su - ${RTUSR} -c \"cd ${basepath} && /opt/IBM/WebSphere/AppServer/bin/wsadmin.sh -lang jython -port ${DmgrPort} -username ${RTUSR} -password Password_Removed -f ${PYSCRIPT} -javaoption -Dpython.path=${basepath} $*\""
 su - ${RTUSR} -c "cd ${basepath} && /opt/IBM/WebSphere/AppServer/bin/wsadmin.sh -lang jython -port ${DmgrPort} -username ${RTUSR} -password ${WAS_Password} -f ${PYSCRIPT} -javaoption -Dpython.path=${basepath} $*"
}
...
OPTS="systemMemory=$systemMemory hostName=${HOST_NAME} threadpoolMaxSize=150 nodeName=${NODE_NAME}"
execPYscript ${OPTS}
