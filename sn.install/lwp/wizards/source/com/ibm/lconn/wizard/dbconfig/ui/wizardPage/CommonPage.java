...
public abstract class CommonPage extends WizardPage {
	...
	//TODO modify by Maxi, sort features!
	public String[] getSortedFeatures(String[] features) {
		String[] result = new String[features.length];
		Map<String, String> labelToName = new HashMap<String, String>();
		for (String str : features) {
			if (Constants.FEATURE_LIBRARY_GCD.equals(str) || Constants.FEATURE_LIBRARY_OS.equals(str)) {
				labelToName.put(Messages.getString(Constants.FEATURE_LIBRARY + ".name") + "." + Messages.getString(str + ".shortname"), str);
			} else {
				labelToName.put(Messages.getString(str + ".name"), str);
			}
		}
		String[] lables = (String[]) labelToName.keySet().toArray();
		if (lables.length != result.length) return features;
		Arrays.sort(lables);
		for (int i = 0; i < lables.length; i++) {
			result[i] = labelToName.get(lables[i]);
		}
		return result;
	}
	
	public List<String> getSortedFeatures(List<String> features) {
		...
		String[] lables = (String[]) labelToName.keySet().toArray();
		if (lables.length != result.size()) return features;
		Arrays.sort(lables);
		for (String str : lables) {
			result.add(labelToName.get(str));
		}
		return result;
	}
	//TODO end
	...
	public void showInputCheckErrorMsg(String message) {
		...
		new MessageDialog(getShell(), title, null, message, ERROR, new String[] { Messages.getString("button.OK.text") }, 0).open();
	}
	...
	public DBWizard getWizard() {
		return (DBWizard) super.getWizard();
	}
