 ...
 * 
 */
public class UninstallNodeTask extends AbstractTask {

	public int run() {
		String feature = DataPool.getValue(ClusterConstant.WIZARD_ID_CLUSTER,
				ClusterConstant.INPUT_RUNNING_FEATURE);
		DataPool.setValue(ClusterConstant.WIZARD_ID_CLUSTER, ClusterConstant.INPUT_clusterNodeProfile,
				LCUtil.getFeature(feature).getProfileName());
		CommandExec ct = CommandExec.create(ClusterConstant.TASK_DELETE_NODE);
		...
		log = LOG_DIR + Constants.FS + "action-uninstall-cluster-setup-"
				+ feature + ".log";
		ct.setOutput(log);
