...
public class MarkManagerTask extends Task {
	private Pattern[] error = { Pattern.compile(".*exception:.*",
	...
	public CommandResultInfo run() {
		...
		if (Constants.OS_WINDOWS.equals(this.osType)) {
			command = WORKDIR + FS + "mark_managers.bat";
			...
			CommandExecuter.run(new String[] { "chmod", "+x",
					WORKDIR + FS + "mark_managers.sh" }, null, this.env,
					this.logPath);
			command = WORKDIR + FS + "mark_managers.sh";
			CommandLogger.log("Execute [" + command + "]");
			commandResult = CommandExecuter.run(command, null, this.env, this.logPath);
		}		
