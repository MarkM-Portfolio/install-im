...
public class LCFileBrowser extends LCWizardInput {

	private Text text;
	private Label label;
	private Button button;
	
	public LCFileBrowser(Composite parent, WizardPageInputData data) {
		super(parent, data);
	}
	
	private void fireButtonSelected() {
		FileDialog directoryDialog = new FileDialog(Display
				.getCurrent().getActiveShell());
		if (text.getText() != null)
			directoryDialog.setFilterPath(text.getText());
		String dirBrowsePanelMsg = getData().getValue(Constants.WIDGET_PROP_FILE_BROWSER_DIALOG_MSG);
		if (CommonHelper.isEmpty(dirBrowsePanelMsg))
			dirBrowsePanelMsg = Messages
					.getString(Constants.DEFAULT_FILE_BROWSER_MSG);
		directoryDialog.setText(dirBrowsePanelMsg); //$NON-NLS-1$
		String value = getData().getValue(Constants.WIDGET_PROP_FILE_BROWSER_EXT_FILTER);
		if(!CommonHelper.isEmpty(value)){
			String[] ext = Util.delimStr(value);
			directoryDialog.setFilterExtensions(ext);
		}
	...
	public void createControl(Composite parent) {
		...
		if(CommonHelper.isEmpty(buttonTxt)){
			this.button.setText(MessageUtil.getLabel("file.browser.button"));
		}else{
	...
	public void setEnable(boolean enable, String... data) {
		...
		if(this.button!=null) this.button.setEnabled(enable);
