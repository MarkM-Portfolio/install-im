 ...
 * 
 */
public class DBWizardDialog extends WizardDialog {
	private Point initialSize = new Point(800, 600);

	public DBWizardDialog(Shell parentShell, DBWizard newWizard) {
	...
	public boolean close() {
		if (this.getTheCancelButton() != null && this.getTheCancelButton().getEnabled()) {
			boolean status = new MessageDialog(getShell(), Messages.getString("dbWizard.window.title"), null, Messages.getString("wizard.close.question.message"), MessageDialog.WARNING, new String[] { Messages.getString("button.YES.text"), Messages.getString("button.NO.text") }, 0).open() == 0;
			if (status)
				return super.close();
			else
				return status;
	...
	protected void nextPressed() {
		BusyIndicator.showWhile(getShell().getDisplay(), new Runnable() {
			public void run() {
				DBWizard wizard = (DBWizard) getWizard();
