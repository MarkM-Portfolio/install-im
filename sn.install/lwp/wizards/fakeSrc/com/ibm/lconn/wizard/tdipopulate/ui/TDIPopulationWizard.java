...
public class TDIPopulationWizard extends LCWizard implements LCAction {
	...
	private void setFinishPanel(CommandResultInfo runCommand) {
		if (runCommand != null) {
			...
			exitMessage = MessageUtil.getMsgWithParameter(
	...
	private void setSSLWidgets() {
		...
		sslCheck.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event arg0) {
				...
				DataPool.setValue(Constants.WIZARD_ID_TDIPOPULATE,
						Constants.INPUT_TDI_LDAP_SERVER_PORT, DefaultValue
								.getLDAPPort(sslEnabled));
				port.updateData();
			}
		});
	}
	...
	private void setFinishPage() {
		...
		page.addPageRenderListener(new Listener() {
			public void handleEvent(Event event) {
				setButtonStyle(Constants.DISABLE_CANCEL);
			}
		});
	}
	...
	private String getFinishString() {
		...
		List<String> optinalTasks = Arrays.asList(DataPool.getValue(wizardId,
				Constants.INPUT_TDI_OPTIONAL_TASK).split(","));
		...
		while (iterator.hasNext()) {
			...
			if ("task.country.csv".equals(entryKey)
					&& !optinalTasks
							.contains(Constants.LDAP_OPTIONAL_TASK_FILL_COUNTRIES)) {
				continue;
			}
			if ("task.department.csv".equals(entryKey)
					&& !optinalTasks
							.contains(Constants.LDAP_OPTIONAL_TASK_FILL_DEPARTMENT)) {
				continue;
			}
			if ("task.organization.csv".equals(entryKey)
					&& !optinalTasks
							.contains(Constants.LDAP_OPTIONAL_TASK_FILL_ORGANIZATION)) {
				continue;
			}
			if ("task.empoyeetype.csv".equals(entryKey)
					&& !optinalTasks
							.contains(Constants.LDAP_OPTIONAL_TASK_FILL_EMPLOYEE)) {
				continue;
			}
			if ("task.worklocation.csv".equals(entryKey)
					&& !optinalTasks
							.contains(Constants.LDAP_OPTIONAL_TASK_FILL_WORK_LOCATION)) {
				continue;
			}
