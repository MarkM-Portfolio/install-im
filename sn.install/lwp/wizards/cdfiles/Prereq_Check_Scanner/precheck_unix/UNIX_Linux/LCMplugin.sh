...
#put value from LCM config to PREREQ_CHECKER config for the current PRODUCT and SYSTEM
copyValue() {
	VAL_TO=$1 ; VAL_FROM=$2
	VALUE=`getValue ${PRODUCT}_${SYSTEMID}_${VAL_FROM} ${LCM_CFG}`
...
DEBUG_FILE="LCM_debug.log"

if [ -f ${DEBUG_FILE} ]; then
	rm -f ${DEBUG_FILE}
fi

LCM_CFG="LCMplugin.cfg"
isWasAgent=

key1="Disk" #COMMON KEYS: 1 2 3 4 6  #LCM KEYS: 5 7 8 9  #TAD KEYS: 10
key2="TEMP"
key3="CIT"
key4="TCD"
key5="MAIN"
...
for ARG in `echo $3 | tr "," "\n"`; do
	case $ARG in
		WASAgent=*)
			isWasAgent=`echo $ARG | awk -F"=" '{print $2}'`
			echo "is-WAS-Agent?: $isWasAgent"
	esac
...
#set config variable name depending on isWasAgent flag
if [ "$isWasAgent" = "true" ] ; then
	DISKVALUE=WASDISK
else
	DISKVALUE=DISK
fi

#write disk value according to isWasAgent flag
copyValue $key1 $DISKVALUE
