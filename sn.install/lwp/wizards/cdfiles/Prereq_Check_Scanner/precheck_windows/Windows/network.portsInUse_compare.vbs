...
'
' File:     Windows/network.portsInUse_compare.vbs
' Version:  1.1.1.7
' Modified: 08/25/11
' Build:    20110825
'
...
function portsInUse(expected, real)
    ...
    If matches.Count=1 Then
        ...
        If Len(endPort)=0 Then
            ...
            If isPortAvailable(startPort,real) Then
                result = "FAIL"
            End If
        Else
            ' Range comparison
            ...
            If intEndPort >= intStartPort Then
                ' Check all numbers in the range.  If any are not in use, fail the check
                For port = intStartPort to intEndPort
                    If isPortAvailable(CStr(port), real) Then
                        result = "FAIL"
                        Exit For
                    End If
                Next
