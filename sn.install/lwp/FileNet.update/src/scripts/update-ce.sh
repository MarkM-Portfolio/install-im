...
log() {
	echo "$*" 2>&1 | tee -a ${fn_ce_update_log}
}

mutedLog() {
	echo "$*" 2>&1 >> ${fn_ce_update_log}
}

doActionWithRC() {
	eval $* >> ${fn_ce_update_log} 2>&1
}
...
usage() {
	...
	log "ce_fp_installer_location: Location of FileNet CPE installer (e.g.: /opt/filenet-fixpack/5.2.1.4-P8CPE-${os_c}-FP004.BIN)"
}
...
validate() {
	...
	tmp_a_size=$((tmp_size-1480340*3))
	if [ $tmp_a_size -lt 0 ]
	then
		log "Error: To install CPE Fixpacks, there should be at least 4GB under directory /tmp. Please try either of the following methods:"
	...
	do
		...
		echo "$temp" >> ${fn_ce_update_log} 2>&1
		log ""
	done
	...
	# config Connections home folder
		firstTime_="true"
		while [ ! -d "${conn_home_path}/FileNet/ContentEngine" ] || [ ! -f "$conn_home_path/tmp/ce_silent_install_unix.txt" ]
		do
			if [ -z "$conn_home_path" ]
			then
				if [ ! "$firstTime_" == "true" ]
				...
				if [ ! "$firstTime_" == "true" ]
				then
					log "Error: $conn_home_path is not the path of Connections Home or there is no FileNet/ContentEngine folder or ce_silent_install_unix.txt in Connections Home."
					log ""
				else
					firstTime_="false"
				fi
	...
	firstTime_="true"
	# ce_os_validator=`echo $ce_fp_installer_location | grep $platform`
	while [ ! -f "$ce_fp_installer_location" ]
	do
		if [ -z "$ce_fp_installer_location" ]
		then
			if [ ! "$firstTime_" == "true" ]
			then
				log "Error: The location of FileNet CPE Fixpack installer is empty."
				log ""
			else
				firstTime_="false"
			fi
			...
			if [ ! "$firstTime_" == "true" ]
			then
				log "Error: $ce_fp_installer_location is not the location of FileNet CPE Fixpack installer."
				log ""
			else
				firstTime_="false"
			fi
    ...
    log ""
		log "Input the installater location of FileNet CPE Fixpack installer (e.g.: /opt/filenet-fixpack/5.2.1.4-P8CPE-${os_c}-FP004.BIN) [$ce_fp_installer_location]:"
		read temp
		if [ ! "$temp" == "" ]
		then
			if [[ $temp == *P8CPE* ]] && [[ $temp == *BIN ]]
				then
				    ce_fp_installer_location=$temp
				else
					log "Error: $temp is not the location of FileNet CE installer or the binary file is wrong."
					firstTime_="true"
			fi
			# ce_os_validator=`echo $ce_fp_installer_location | grep $platform`
		fi
...
if [ -z "${fn_ce_update_log}" ]
then
	fn_ce_update_log=${fnUpdateScriptPath_}/fn-ce-update.log
	rm -rf $fn_ce_update_log
	create_log=$?
	if [ 0 -ne $create_log ]
	then
	  log ""
...
cmd_ce="$ce_fp_installer_location -i silent"
cmd_ce="$cmd_ce -f $conn_home_path/tmp/ce_silent_install_unix.txt"

export WAS_HOME=${was_dm_path}
export MY_HOME=${conn_home_path}
export set_fn_anonymous=n

if [[ ! -x "$ce_fp_installer_location" ]]
then
	  log ""
		log "user does NOT have permission to run $ce_fp_installer_location."
		log ""
		exit 1
fi

if [ $platform == "AIX" ]; then
	 chmod u+x $ce_fp_installer_location
	 if [ 0 -ne $? ]
	 then
		 log ""
		 log "user does NOT have permission to run $ce_fp_installer_location."
...
log ""
log "installing FileNet CPE Fixpack $ce_fp_installer_location ..."
log ""
...
${was_dm_path}/bin/wsadmin.sh -lang jython -conntype SOAP \
    ...
    -javaoption "-Xmx512m" \
    -f "${conn_home_path}/FileNet.update/scripts/install_fn_fp.py" "ce" "${cmd_ce}" >> ${fn_ce_update_log} 2>&1
ce_return_code=$?
if [ $ce_return_code -eq 99 ]
then
	log "Node agent is not start yet, please make sure all node agents are start up before updating FileNet"
	exit 1
fi
if [ 0 -ne $ce_return_code ]
then
	log "return code: $ce_return_code"
	log "Error: Please read the log for detail of the problem, it is ${fn_ce_update_log}."
	exit 1
fi

log "updating WAS Admin Username..."
$JAVA_HOME/bin/java -jar ${conn_home_path}/lib/lccfg.jar ${conn_home_path}/FileNet/ContentEngine/tools/configure/profiles/CCM/applicationserver.xml \
...
log ""
log "Deploying FileNet Content Engine ear ..."
log ""
...
${was_dm_path}/bin/wsadmin.sh -lang jython -conntype SOAP \
    ...
    -f "${conn_home_path}/FileNet.update/scripts/deploy_fn_apps.py" "${conn_home_path}/cfg.py" "deploy_ce" >> ${fn_ce_update_log} 2>&1
ce_deploy_code=$?
if [ 0 -ne $ce_deploy_code ]
then
    log "task return code: $ce_deploy_code"
		log "IBM FileNet Engine deploy failed, exit."
		exit 1
else
	  log "IBM FileNet Content Engine have been deployed successfully."
fi
...
${was_dm_path}/bin/wsadmin.sh -lang jython -conntype SOAP \
    ...
    -wsadmin_classpath "${conn_home_path}/lib/lccfg.jar" \
		-javaoption "-Dpython.path=${conn_home_path}/lib" \
    -f "${conn_home_path}/FileNet.update/scripts/deploy_fn_apps.py" "${conn_home_path}/cfg.py" "config_ce" >> ${fn_ce_update_log} 2>&1
ce_config_code=$?
if [ 0 -ne $ce_config_code ]
then
    log "IBM FileNet Content Engine config failed, exit."
		exit 1
else
	  log "IBM FileNet Content Engine have been configured successfully."
fi

log "IBM FileNet Content Engine have been upgraded successfully."
