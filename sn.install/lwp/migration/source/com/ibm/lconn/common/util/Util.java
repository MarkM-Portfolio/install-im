...
public class Util {
	...
	public static int indexOf(String[] strArry, String str){
		...
		for (int i = 0; i < strArry.length; i++) {
			if(ObjectUtil.equals(strArry[i], str))
				return i;
		}
		return -1;
	}
	...
	public static<T> int indexOf(List<T> tList, T t){
		...
		for (int i = 0; i < tList.size(); i++) {
			if(ObjectUtil.equals(tList.get(i), t))
				return i;
		}
		return -1;
	}
	...
	public static String addSelection(String value,
		...
		if(ObjectUtil.isEmpty(value)) return addingSelection;
	...
	public static File getFile(String dest) {
		return new File(new File(dest).getAbsolutePath());
	}
