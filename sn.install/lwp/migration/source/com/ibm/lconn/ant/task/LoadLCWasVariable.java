 ...
 * 
 */
public class LoadLCWasVariable extends BaseTask {
	...
	public void execute() {
		...
		for (int j = 0; j < features.length; j++) {
			...
			String profile = getProject().getProperty(feature + ".ProfileName");
			log("Profile name = {0}", profile);
			String loadLevel = getLevel().toLowerCase();
			String cell = getProject().getProperty(feature + ".CellName.migration");
			String node = null;
			String server = null;
			log("Cell name = {0}", cell);
			if(!"cell".equals(loadLevel)){
				node = getProject().getProperty(feature + ".NodeName");
				log("Node name = {0}", node);
				if(!"node".equals(loadLevel)){
					server = getProject().getProperty(feature+".ServerName");
