    ...
    extends efixBaseInstaller
{
    ...
    // Instantor ...
    //
    // public efixBatchUpdater(WPProduct, WPHistory, Notifier, IOService);
    ...
    public boolean prepareImages(Vector updateImages)
    {
        ...
        for ( int imageNo = 0; !failed && (imageNo < imageCount); imageNo++ ) {
            ...
            try {
                nextImage.prepareEFixDriver(); // throws IOException, FactoryException
    ...
    public boolean testInstallPrerequisites(Vector updateImages,
                                            Vector installOrder,
                                            Vector errors, Vector supersededInfo)
    {
        Vector efixDrivers = new Vector();
        ...
        boolean result = checker.testEFixInstallation(efixDrivers, driverInstallOrder, errors, supersededInfo);

        for ( int imageNo = 0; imageNo < imageCount; imageNo++ ) {
            efixDriver nextDriver = (efixDriver) driverInstallOrder.elementAt(imageNo);
    ...
    protected int countInstallableComponents(efixImage anImage)
    {
        ...
        while ( useComponentNames.hasNext() ) {
            ...
            if ( isInstallable(useEFixId, nextComponentName) )
                count++;
        }
